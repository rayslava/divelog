...
\usepackage{forloop}
\usepackage{tikz}
\usepackage[margin=3mm,left=-1.3cm, right=1.8cm,portrait]{geometry}
\usepackage{pgfpages}
\usepackage{textcomp}
\usepackage{xcolor}
\usepackage{utfsym}
...
% \usepackage[grid, gridunit=mm, gridcolor=blue!30, subgridcolor=blue!15]{eso-pic}
...
\begin{document}
\newgeometry{margin=3mm,left=1.5cm}
\newcounter{counter}
...
\forloop{counter}{0}{\value{counter} < 2}{
  ...
  \begin{tikzpicture}[node distance=0.4em,text width=6em]
    ...
    % Feeling part
    \node [align=left, below = 1.5em of othtank, xshift=1.3em, text width=26em] (feeling) {\textcolor{grayedcaption}{Feel} $\square$~Hot~$\square$~Warm~$\square$~Ok~$\square$~Chill~$\square$~Cold~$\square$~Freezing};
    \begin{pgfonlayer}{background}
      % Compute a few helper coordinates
      \pgfmathsetmacro{\sp}{0.15}
    ...
    % Water entry part
    \node [align=left, below = 1.5em of feeling, text width=26em] (entry) {\textcolor{grayedcaption}{Entry\hspace{2em}} $\square$~Shore~$\square$~Deck~$\square$~Boat~$\square$~Zodiac};
    \begin{pgfonlayer}{background}
      % Compute a few helper coordinates
      \pgfmathsetmacro{\sp}{0.15}
