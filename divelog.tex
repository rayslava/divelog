...
\begin{document}
\newgeometry{margin=3mm,left=1.5cm,portrait}
\newcounter{counter}
...
\forloop{counter}{0}{\value{counter} < 2}{
  ...
  \begin{tikzpicture}[node distance=0.4em,text width=6em]
    ...
    \node [align=center, below = 1.5em of othtank, xshift=1.3em, text width=26em] (feeling) {\textcolor{grayedcaption}{Feel} $\square$~Hot~$\square$~Warm~$\square$~Ok~$\square$~Chill~$\square$~Cold~$\square$~Freezing};
    \begin{pgfonlayer}{background}
      % Compute a few helper coordinates
      \pgfmathsetmacro{\sp}{0.15}
