...
\forloop{counter}{0}{\value{counter} < 2}{
  ...
  \begin{tikzpicture}[node distance=0.4em,text width=6em]
    ...
    \begin{pgfonlayer}{background}
      ...
      \path (dry.south -| dry.east)+(+\sp,-\sp+.12) node (b) {};
      \path[rounded corners, draw=dashbox, dashed]
      (a) rectangle (b);

      \pgfmathsetmacro{\sp}{0.05}
      \path (long.west |- long.north)+(-\sp,\sp-.03) node (a) {};
    ...
    {\textcolor{grayedcaption}{Photo/video~}$\square$~GoPro~\hrulefill};
    \begin{pgfonlayer}{background}
      % Compute a few helper coordinates
      \pgfmathsetmacro{\sp}{0.15}
  ...
  \begin{minipage}{0.89\linewidth}
    \hfill\\
    \phantom{}\rule{.98\textwidth}{0.5pt}\\
    \phantom{}\rule{.98\textwidth}{0.5pt}\\
    \phantom{}\rule{.98\textwidth}{0.5pt}\\
    \phantom{}\rule{.98\textwidth}{0.5pt}\\
    \phantom{}\rule{.98\textwidth}{0.5pt}\\
